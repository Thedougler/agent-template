---
name: skill-lookup
description: "Search, retrieve, and install Agent Skills from the prompts.chat registry using MCP tools. Use when the user asks to find skills, browse skill catalogs, install a skill for Claude, or extend Claude's capabilities with reusable AI agent components."
license: MIT
---
...
4. Install by saving files to `.github/skills/{slug}/` and verify the SKILL.md exists
...
2. Create the directory `.github/skills/{slug}/`
3. Save each file to the appropriate location:
   - `SKILL.md` → `.github/skills/{slug}/SKILL.md`
   - Other files → `.github/skills/{slug}/{filename}`
